	...
	var loadConfigFile = function (type) {
        ...
        if (type === 'user') {
			...
			if(_url_.param('config')) {
				...
				url = fm.settings.configUrl ? fm.settings.configUrl : fm.settings.baseUrl + '/config/filemanager.config.default.json'; // if configUrl is defined
			}
		} else {
			url = fm.settings.baseUrl + '/config/filemanager.config.default.json';
		}
        console.log("fm.settings.baseUrl:" + fm.settings.baseUrl);
		return $.ajax({
			type: 'GET',
			url: url,
			dataType: 'json',
