...
(function($) {
    ...
    $.fn.simpleCropper = function (formName) {
        ...
        //if (mevcutResim != "")
        //{
        //    $('.cropme').css('background-image', 'url("' + mevcutResim + '")');
        //}
    ...
    this.click(function () {
	  ...
	  console.log("1:"+file_display_area);
      $('#fileInput').click();
    });
    ...
    function preview() {
      ...
      //aþaðýdaki satýr yerine kademeli olarak resim küçültüp croplayan kod bloðu eklenmiþtir
      //context.drawImage(current_image, (x1 * kw), (y1 * kh), (sw * kw), (sh * kh), 0, 0, aspX, aspY);

      // kod bloðu baþý
      var gecerliStep = Math.floor(Math.log(sw * kw / canvas.width) / Math.log(2))
      if (gecerliStep > 0) {
          var tempImageWidth = sw * kw;
          var tempImageHeight = sh * kh;
          var tempCanvasWidth = sw * kw * 0.5;
          var tempCanvasHeight = sh * kh * 0.5;

          var oc = document.createElement('canvas'),
          octx = oc.getContext('2d');
          oc.width = sw * kw * 0.5;
          oc.height = sh * kh * 0.5;
          octx.drawImage(current_image, (x1 * kw), (y1 * kh), tempImageWidth, tempImageHeight, 0, 0, tempCanvasWidth, tempCanvasHeight);
          for (i = 1; i < gecerliStep; i++) {
              tempImageWidth = tempCanvasWidth;
              tempImageHeight = tempCanvasHeight;
              tempCanvasWidth = tempCanvasWidth * 0.5;
              tempCanvasHeight = tempCanvasHeight * 0.5;
              octx.drawImage(oc, 0, 0, tempImageWidth, tempImageHeight, 0, 0, tempCanvasWidth, tempCanvasHeight);
          }
          context.drawImage(oc, 0, 0, tempCanvasWidth, tempCanvasHeight, 0, 0, canvas.width, canvas.height);
      } // croplanacak bölüm canvastan büyükse
      else {
          context.drawImage(current_image, (x1 * kw), (y1 * kh), (sw * kw), (sh * kh), 0, 0, aspX, aspY);
      } // croplanacak bölüm canvastan küçükse veya eþitse
      //kod bloðu sonu
      ...
      input.setAttribute("class","hdnimginput");
      input.setAttribute("value", dataUrl);
	   console.log("2:"+file_display_area);
	    console.log("3:"+ file_display_area.context.id);
      input.setAttribute("name", "cropped_" + file_display_area.context.id);
